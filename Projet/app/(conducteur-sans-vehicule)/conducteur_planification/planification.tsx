...
const CalendarPage: React.FC = () => {
  ...
  const handleFormSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (title && startDate && startTime && endDate && endTime && departure && arrival) {
          ...
          {showForm && (
            <form onSubmit={handleFormSubmit} onKeyPress={handleKeyPress}>
              <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center">
                <div className="bg-blue-100 m-1 p-8 rounded shadow-lg w-full max-w-4xl">
                  ...
                  <div className="mt-4 flex justify-between">
                    <button type='submit' className="bg-blue-500 text-white px-4 py-2 rounded">{selectedEvent ? 'Modifier' : 'Valider'}</button>
                    ...
                    <button onClick={handleCancel} className="bg-gray-500 text-white px-4 py-2 rounded">Annuler</button>
                  </div>
                </div>
              </div>
            </form>
